...
import { CreditCard, Banknote, Smartphone, Building, CheckCircle } from 'lucide-react';
...
const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const handlePayment = async () => {
    ...
    try {
      await onPaymentComplete(transaction.id, selectedPaymentMethod, paymentDate);
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <p className="text-lg font-bold text-blue-600 mt-2">{formatCurrency(transaction.amount)}</p>
          </div>

          <div>
            <Label className="text-base font-medium">Data do Pagamento</Label>
            <div className="mt-2">
              <Input type="date" value={paymentDate} onChange={(e) => setPaymentDate(e.target.value)} className="w-full" max={new Date().toISOString().split('T')[0]} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2 mt-2">
              {paymentMethods.map((method) => {
                ...
                return (
                  <button key={method.id} onClick={() => setSelectedPaymentMethod(method.id)}
                    ...
                    {selectedPaymentMethod === method.id && <CheckCircle className="h-4 w-4 text-blue-500 ml-auto" />}
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex space-x-3">
            ...
            <Button onClick={handlePayment} disabled={!selectedPaymentMethod || !paymentDate || isProcessing} className="flex-1">
